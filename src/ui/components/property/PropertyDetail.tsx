...
function PropertyDetailInner({ property, onBack, onWhatsApp, standalone }: Props) {
  ...
  // ★ Direct native click on sticky bar — the ENTIRE bar opens the modal
  useEffect(() => {
    const stickyBar = stickyBarRef.current;
    if (!stickyBar) return;

    const openModal = () => setLeadModalOpen(true);
    stickyBar.addEventListener('click', openModal);
    return () => stickyBar.removeEventListener('click', openModal);
  }, []);

  // ★ Direct native click on PriceCard CTA button
  useEffect(() => {
  ...
  return (
    <div ref={containerRef} style={{ background: 'var(--main-bg, #FAF8F3)', minHeight: '100vh', color: 'var(--main-text, #111113)' }}>
      ...
      {/* Mobile sticky CTA bar — ENTIRE bar is clickable */}
      <div
        ref={stickyBarRef}
        ...
        style={{
          ...
          cursor: 'pointer',
          WebkitTapHighlightColor: 'rgba(37,211,102,0.1)',
          userSelect: 'none',
        }}
      >
